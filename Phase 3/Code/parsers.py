...
#Function to map location no to location

def location_id_mapper():
    locationIdMapper = defaultdict(int)
    tree = ET.parse('../devset_topics.xml')
    topics = tree.findall('topic')
    for topic in topics:
        locationIdMapper[int(topic.find('number').text)] = topic.find('title').text
    return locationIdMapper

#Function to generate dataframe for all images of given location using all the model
def parse_csv_location_all_model(location_name, models):
    all_data = []
    for model in models:
        df = pd.read_csv("../descvis/img/" + location_name+ " " + model + ".csv", header = None) 
        df['location'] = location_name
        df.rename(columns={0: 'img_id'}, inplace=True) 
        df.set_index(['img_id'], inplace=True, drop=True)
        all_data.append(df)
    location_df = pd.concat(all_data, axis = 1)
    return location_df
...
def parse_all_location_models_2(location_dict):
    all_data = []
    # models = ['CM', 'CM3x3', 'CN', 'CN3x3', 'CSD', 'GLRLM', 'GLRLM3x3', 'HOG', 'LBP', 'LBP3x3']
    models = ['CM', 'CM3x3', 'CN', 'CN3x3', 'CSD', 'GLRLM', 'GLRLM3x3', 'HOG', 'LBP', 'LBP3x3']
    for model in models:
    ...
    csv_df = pd.concat(all_data, axis = 1, sort = False)
    csv_df = csv_df.fillna(0)
    # df.set_index(['img_id'], inplace=True, drop=True)
    return csv_df
